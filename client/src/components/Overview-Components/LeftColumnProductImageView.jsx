...
function LeftColumnProductImageView({
  selectedStyle,
  fullscreenToggle,
  setFullscreenToggle,
}) {
  let renderMainImages;
  const thumbnailsLength = selectedStyle.photos
    ? selectedStyle.photos.length
    : 0;
  ...
  function thumbClickUp() {
    setThumbRange([
      ...
      thumbRange[1] === thumbnailsLength - 7
        ? thumbnailsLength - 7
        : thumbRange[1] - 1,
    ]);
  }
  ...
  function thumbClickDown() {
    setThumbRange([
      thumbRange[0] === thumbnailsLength - 7
        ? thumbnailsLength - 7
        : thumbRange[0] + 1,
      thumbRange[1] === thumbnailsLength - 1
        ? thumbnailsLength - 1
        : thumbRange[1] + 1,
    ]);
  }
  ...
  if (selectedStyle.photos) {
    renderMainImages = selectedStyle.photos.map((photo, idx) => {
      const displayUrl =
        photo.url === null ? 'No-Image-Placeholder.svg' : photo.url;
      return (
        <RenderMainImages
          key={idx}
          idx={idx}
  ...
  if (selectedStyle.photos) {
    renderMainIndicators = selectedStyle.photos.map((photo, idx) => (
      <MainIndicators key={idx} idx={idx} />
    ));
  } else {
  ...
  return (
    <div className={`${fullscreenToggle ? 'col-lg-12 h-100 g-0' : 'col-lg-8'}`}>
      ...
      >
        ...
        <FullscreenButton
          type="button"
          className={`fullscreen-icon ${zoom ? 'd-none' : ''}`}
          onClick={handleFullscreen}
        >
          <img
            alt="biFullscreen"
            id="biFullscreen"
            src="assets/biFullscreen.svg"
          />
        </FullscreenButton>
        <div className={`${zoom ? 'd-none' : ''}`}>
          <CarouselIndicators
            className={`carousel-indicators ${
              fullscreenToggle
                ? 'fullscreenView'
                : 'normalView d-flex flex-column justify-content-evenly'
            }`}
          >
            <img
              src="assets/bsChevronCompactUp.svg"
              alt="BsChevronCompactUp"
              className={`thumbnailArrows ${
                thumbnailsLength > 7 && thumbRange[0] !== 0 ? '' : 'd-none'
              } ${fullscreenToggle ? 'd-none' : ''}`}
            ...
            <img
              ...
              className={`thumbnailArrows ${
                thumbnailsLength > 7 && thumbRange[1] !== thumbnailsLength - 1
                  ? ''
                  : 'd-none'
              } ${fullscreenToggle ? 'd-none' : ''}`}
